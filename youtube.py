...
from streamlit_option_menu import option_menu as bar
...
with st.sidebar:
    selected = bar(None, ["Home", "Extract and Mutate", "view"],
...
if selected == "Home":
    ...
    # st.title(':green[Data Harvesting]')
    ...
    with col1:
        ...
        st.write(
            '(:green[Note:- This zone **collect data** by using channel id and **stored it in the :blue[MongoDB] database]**.)')
        channel_id = st.text_input('**channel id please**')
        ...
        if Get_data or st.session_state.Get_state:
            ...
            api_version = 'v3'
            api_key = 'AIzaSyBFlJlhp2g4fWN2FbTT6w6KZzHWdKTIgPs'
    ...
    with col2:
        st.header(':black[Data Migrate zone]')
        st.write('''(:green[Note:- This zone specific channel data **Migrate to :blue[MySQL] database from  :green[MongoDB] database** depending on your selection,
        ...
        if Migrate or st.session_state_migrate_sql:
            ...
            engine = create_engine('mysql+mysqlconnector://root:mysql@localhost/youtube_db', echo=False)
    ...
    connect_for_question = pymysql.connect(host='localhost', user='root', password='mysql', db='youtube_db')
    ...
    # # col4.markdown("## :blue[Domain] : Technology Media")
    #
    # col3.markdown("## :blue[Technologies used] : Python,MongoDB, Youtube Data API, MySql, Streamlit")
    # col3.markdown("#   ")
    col3, col4 = st.columns(2, gap='medium')


    # col4.markdown("## :blue[Domain] : Techncology Media")

    col3.markdown(
        "<h1 style='text-align: center; color: blue;'>Technologies used</h2>",
        unsafe_allow_html=True)
    col3.markdown("<h2 style='text-align: center;'>Python, MongoDB, Youtube Data API, MySQL, Streamlit</h1>", unsafe_allow_html=True)
